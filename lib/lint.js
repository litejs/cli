...
module.exports = function(opts) {
	...
	, jsFiles = files.filter(function(f) { return f.endsWith(".js") })
	, formatNeeded = [].concat(
		validate(files, "css", isValidCss),
		validate(files, "js", isValidJs),
		validate(files, "json", isValidJson),
		validate(files, "jsonc", isValidJson)
	)
	if (opts.jshint && jsFiles[0]) try {
		child.execFileSync("npx", ["jshint", "--config=" + opts.jshint].concat(jsFiles), { stdio: ["ignore", 1, 2]})
	} catch(e) {
		exit = 1
	}
