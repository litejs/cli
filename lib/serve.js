...
module.exports = function(opts) {
	...
	return http.createServer(function(req, res) {
		...
		if (req.method === "GET") {
			sendFile(res, req.url.split("?")[0], opts.spa)
		} else {
			var body = ""
			req.on("data", buf => body += buf).on("end", () => {
				console.log(body)
				res.statusCode = 204
				res.end()
			})
