...
//-    --assetsKeep    Glob patterns for assets to keep original name eg "*.webmanifest,robots.txt"
//-                    In HTML, mark individual assets with [keep-name] attribute
//-                    In .ui templates, append [keep-name] or :keep-name after [src=...] selector
...
function write(dir, name, content, el) {
	...
	if (name.indexOf("{h}") > -1) {
		hash = child.execFileSync("git", ["hash-object", "-w", "--stdin"], { input: content }).toString("utf8").trim()
		hash = child.execFileSync("git", ["rev-parse", "--short=1", hash]).toString("utf8").trim()
...
function html(opts, next) {
	...
	, assetsKeepRe = opts.assetsKeep && opts.assetsKeep.length ? new RegExp("^(?:" + opts.assetsKeep.map(function(p) {
		return cli.globRe(p, true)
	}).join("|") + ")$") : null
	...
	if (opts.cssPrune !== false) {
		$$("[class]").forEach(function(el) { addClasses(el.className) })
		...
		// LiteJS ui sets orientation and breakpoint classes on <html> at runtime,
		// they never appear in markup but are used as ancestors in eg `.md .md-w6`
		addClasses("port land " + breakpoints)
	...
	function assetUrl(u, keep) {
		...
		, out = (keep || assetsKeepRe && assetsKeepRe.test(parts[0]) ? parts[0] : opts.assets || "{h}.{ext}")
			.replace("{h}", hash || name)
			.replace("{ext}", ext)
			.replace("{name}", name)
	...
	function cpOut(el) {
		...
		if (urlFn && !httpRe.test(name) && !el.matches("[inline],[min]")) {
			el[el.src ? "src" : "href"] = urlFn(cleanName, el.hasAttribute("keep-name"))
		}
		el.removeAttribute("keep-name")
	}
	...
	function curl(name, el) {
		...
		if (!data.body) {
			data.body = child.execFileSync("uglifyjs", ["--beautify"], {
				input: child.execFileSync("curl", [name])
			}).toString("utf8")
		}
	...
	function read(_name) {
		...
		, ext = getExt(name)
		, extTo = getExt(el.getAttribute("min") || el) || ext
	...
	function minimize(el, _opts) {
		...
		, ext = (
			el.getAttribute("inline") === "ui" ? "ui" :
			el._min ? getExt(el._min) || "js" :
			el.type === "ui" ? "ui" :
			"js"
		)
		if (ext === "json") {
...
function readHashes(root) {
	...
	var _opts = root ? { cwd: root } : undefined
	child.execFileSync("git", ["add", "-u"], _opts)
	child.execFileSync("git", ["ls-files", "-sz", "--abbrev=1"], _opts)
...
function hashFile(file) {
	try {
		var hash = child.execFileSync("git", ["hash-object", "--", file]).toString("utf8").trim()
		return (fileHashes[file] = child.execFileSync("git", ["rev-parse", "--short=1", hash]).toString("utf8").trim())
	} catch(e) {
		return ""
	}
}
...
function addClasses(str) {
	str.split(/[\s;=,?]+/).forEach(function(c) { if (c) classes.add(c) })
}
...
function parseView(content, extTo, lastMinEl, urlFn) {
	...
	if (classes) arr.forEach(function(line) {
		...
		// %ui { breakpoints: "sm,601=md" } overwrites the default breakpoint classes
		line.replace(/\bbreakpoints\s*:\s*(["'])(.*?)\1/g, function(_, q, map) { breakpoints = map })
	})
	...
	if ((line = arr.join("\n"))) {
		line = line.replace(/\[src=(['"]?)([^"'\]\s]+)\1\](\[keep-name\]|:keep-name)?/g, function(all, q, val, keep) {
			return "[src=" + q + (urlFn ? urlFn(val, !!keep) : val) + q + "]"
		})
