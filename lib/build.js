...
function build(opts) {
	...
	if (ext === "html") {
		...
		cli.mkdirp(outDir)
		readHashes(outDir)
		writeResult(jsMin({files: opts._.map(defMap).map(require.resolve, require)}))
	} else {
...
function html(opts, next) {
	...
	pendingWrites.forEach(function(w) {
		write(w.dir, w.name, w.content, w.el)
	})
	...
	function assetUrl(u) {
		...
		var frag = u.indexOf("#") > -1 ? u.slice(u.indexOf("#")) : ""
		u = frag ? u.slice(0, u.indexOf("#")) : u
		var src = path.resolve(inDir, u)
		, hash = fileHashes[src] || fileHashes[inDir + u] || hashFile(src)
		, ext = u.split(".").pop()
		, name = path.basename(u, "." + ext)
		, out = (opts.assets || "{h}.{ext}")
			.replace("{h}", hash || name)
			.replace("{ext}", ext)
			.replace("{name}", name)
		cli.cp(src, path.join(outDir, out))
		return out + frag
	}
	function cpOut(el) {
		var name = getSrc(el)
...
function readHashes(root) {
	...
	.toString("utf8").split("\0").map(function(line) {
		line = line.split(/\s+/)
		if (line[1]) {
			fileHashes[root + line[3]] = line[1]
			fileHashes[path.resolve(root, line[3])] = line[1]
		}
	})
}
...
function parseView(content, extTo, lastMinEl, urlFn) {
	...
	var out = ""
	if ((line = map["%css"])) {
		if (lastMinEl.css) lastMinEl.css.textContent += line
	...
	if ((line = arr.join("\n"))) {
		...
		if (lastMinEl.ui && extTo !== "ui") lastMinEl.ui.textContent += line
