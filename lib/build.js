...
function build(opts) {
	...
	function writeResult(output) {
		...
		out.split(",").forEach(function(out) {
			...
			, outFile = out.replace(/.*\//, "")
			var bytes = write(outDir, outFile+"?{h}", output)
			console.log("# Write %s %s", outFile, bytes)
			totalBytes += bytes
		})
		...
		console.log("# Total %s bytes", totalBytes)
	}
}
...
function html(opts, next) {
	...
	function assetUrl(u) {
		...
		console.log("# Copy %s -> %s", parts[0], out)
