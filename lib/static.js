...
function start(opts) {
	...
	Object.keys(ui.views).forEach(function(route) {
		...
		if (view.scan) return viewWithData.push(view)
		if (route[0] === "#" || route.indexOf("{") > -1 || view === ui.home) return
		ui.show(route)
		var outName = route + "/index.html"
		createFile(outName)
	})

	console.log("viewWithData", viewWithData)

	viewWithData.forEach(function(view) {
		cli.ls(view.scan).forEach(function(file) {
			var slug = view.r.replace(/\{.*}/, file.replace(/^.*\/|\.[^.]+$/g, ""))
