...
module.exports = function(opts) {
	...
	try { lastTag = child.execSync("git describe --tags --abbrev=0", { stdio: ["pipe", "pipe", "ignore"] }).toString("utf8").trim() } catch(e) {}

	var logRange = lastTag ? lastTag + "..@" : ""
	, group = [
		...
		{ name: "Breaking Changes",  re: /\bbreak[ei]/i,
			log: child.execFileSync("git", ["log", "-z", "--grep", "break", "-i"].concat(logRange ? [logRange] : []))
	...
	if (opts.install) cli.rmrf("node_modules")
	run("install", "npm install", "dependencies can not be installed")
	...
	if (opts._[0] || !opts.rewrite && com.version === cur.version) {
		...
		opts.setVersion.forEach(function(target) {
			var parts = target.split("#")
			, file = path.resolve(parts[0])
			, keys = parts[1].split(".")
			, content = cli.readFile(file)
			if (/\.toml$/.test(parts[0])) {
				var section = keys.length > 1 ? keys.slice(0, -1).join(".") : ""
				, key = keys[keys.length - 1]
				, re = RegExp((section ? "(\\[" + section.replace(/\./g, "\\.") + "\\][\\s\\S]*?" : "(") + key + "\\s*=\\s*)(\"[^\"]*\")")
				cli.writeFile(file, content.replace(re, "$1\"" + cur.version + "\""))
			} else {
				var obj = JSON.parse(content)
				, ref = obj
				for (var j = 0; j < keys.length - 1; j++) ref = ref[keys[j]]
				ref[keys[keys.length - 1]] = cur.version
				cli.writeFile(file, JSON.stringify(obj, null, "  ") + "\n")
			}
		})
	}
	if (opts.rewrite) {
		child.execFileSync("git", ["rebase", "--onto", lastTag + "~1", lastTag])
		child.execFileSync("git", ["cherry-pick", lastTag])
	} else if (opts.tag) {
		try {
			child.execFileSync("git", ["rev-parse", "-q", "--verify", "v" + cur.version])
			console.error("\nfatal: git tag exists?! Ignore with --rewrite option.")
			process.exit(1)
		} catch(e) {}
	}
	...
	msg = "# All commits:\n"
	child.execFileSync("git", ["log", "--pretty=format:%s (%aN)"].concat(logRange ? [logRange + (opts.rewrite ? "~1" : "")] : opts.rewrite ? ["~1"] : []))
	...
	if (opts.rewrite) msg += child.execFileSync("git", ["log", "-1", "--no-show-signature", "--format=%B", "v" + cur.version]).toString("utf8")
	...
	child.spawn("git", ["commit", "-a", "-e", "-F", ".git/COMMIT_EDITMSG"].concat(opts.rewrite ? ["--amend"] : ["--"]), { stdio: "inherit" })
	.on("exit", function(code) {
		if (code !== 0) return console.error("\nAborted (editor exit %s)", code)
	...
	function run(opt, cmd, err, flag) {
		if (cmd && opts[opt]) try {
			;(Array.isArray(cmd) ? cmd : [cmd]).forEach(function(cmd) {
				...
				log(child.execFileSync(parts[0], parts.slice(1)))
			})
		} catch (e) {
			log(e.stdout)
			log(e.stderr)
